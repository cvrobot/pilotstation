...
import com.wingoflights.pilotstation.api.client.apis.CalibrationApi;
import com.wingoflights.pilotstation.api.client.apis.ControlApi;
import com.wingoflights.pilotstation.api.client.apis.ExperimentalApi;
import com.wingoflights.pilotstation.api.client.apis.FollowApi;
import com.wingoflights.pilotstation.api.client.apis.MissionApi;
...
import com.wingoflights.pilotstation.api.lib.drone.property.Battery;
...
import com.wingoflights.pilotstation.api.lib.drone.property.Signal;
...
public class Drone {
    ...
    private ControlClient serviceMgr;
    ...
    void init(ControlClient controlClient, Handler handler) {
        this.handler = handler;
        this.serviceMgr = controlClient;
    ...
    synchronized void start() {
        if (!serviceMgr.isClientConnected())
        ...
        try {
            droneApi = serviceMgr.getpsServices().registerDroneApi(this.apiListener, serviceMgr.getApplicationId());
    ...
    synchronized void destroy() {
        ...
        try {
            if (isStarted(droneApi)) {
                ...
                serviceMgr.getpsServices().releaseDroneApi(droneApi);
            }
    ...
    public boolean performAsyncActionOnHandler(Action action, Handler handler, AbstractCommandListener listener) {
        ...
        if (isStarted(droneApi)) {
            try {
                Log.i("drone","performAsyncActionOnHandler" + action.getType());
    ...
    public boolean isConnected() {
        final IDroneApi droneApi = droneApiRef.get();
        State droneState = getAttribute(AttributeType.STATE);
        Log.d(TAG, "isStarted(droneApi):" + isStarted(droneApi) + " droneState.isConnected():" + droneState.isConnected());
