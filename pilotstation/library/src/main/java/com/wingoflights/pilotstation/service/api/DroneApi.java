...
import com.wingoflights.pilotstation.api.lib.drone.action.CameraActions;
import com.wingoflights.pilotstation.api.lib.coordinate.LatLongAlt;
import com.wingoflights.pilotstation.api.lib.drone.action.ConnectionActions;
...
public final class DroneApi extends IDroneApi.Stub implements DroneInterfaces.OnDroneListener, DroneInterfaces.AttributeEventListener,
    ...
    private final PilotStationService service;

    private ConnectionParameter connectionParams;

    DroneApi(PilotStationService ptService, IApiListener listener, String ownerId) {

        this.service = ptService;
        this.context = ptService.getApplicationContext();
        ...
        try {
            ...
            ptService.releaseDroneApi(this.ownerId);
        }
    ...
    private void checkForSelfRelease() {
        ...
        if (!apiListener.asBinder().pingBinder()) {
            ...
            this.context.startService(new Intent(this.context, PilotStationService.class)
                    .setAction(PilotStationService.ACTION_RELEASE_API_INSTANCE)
                    .putExtra(PilotStationService.EXTRA_API_INSTANCE_APP_ID, this.ownerId));
        }
    }
    ...
    public void executeAction(Action action, ICommandListener listener) throws RemoteException {
        ...
        switch (type) {
            ...
            default:
                if (droneMgr != null) {
                    Log.i("droneapi", "droneMgr.executeAsyncAction" + action.getType());
